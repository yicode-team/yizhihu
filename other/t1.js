...
function registerMenuCommand() {
    ...
    for (let i = 0; i < menu_ALL.length; i++) {
        ...
        if (menu_ALL[i][0] === 'menu_customBlockUsers') {
            ...
            menu_ID[i] = GM_registerMenuCommand(`#️⃣ ${menu_ALL[i][1]}`, function () {
                menu_setting('checkbox', menu_ALL[i][1], menu_ALL[i][2], true, [menu_ALL[i + 1], menu_ALL[i + 2], menu_ALL[i + 3], menu_ALL[i + 4]]);
            });
        ...
        if (event.target == this) {
            ...
            if (rightButton) {
                ...
                if (!answerCollapseButton_) {
                    for (let el of document.querySelectorAll('.List-item, .Card.AnswerCard')) {
                        // 遍历所有回答主体元素
...
function blockUsers(type) {
                    ...
                    if (item) {
                        ...
                        if (item) {
                            ...
                            if (userid && footer && !footer.lastElementChild.dataset.name) {
                                ...
                                footer.lastElementChild.onclick = function () {
                                    blockUsers_button_add(this.dataset.name, this.dataset.userid, false);
                                };
                            }
                        }
                    }
    ...
    function blockUsers_button_people() {
        ...
        for (let num = 0; num < users.length; num++) {
            ...
            if (users[num] === name) {
                // 已存在
                document.querySelectorAll('.Button.Button--primary.Button--red').forEach(function (item) {
...
function blockKeywords(type) {
    ...
    function blockKeywords_(className1, className2) {
        ...
        function blockKeywords_now() {
            if (location.pathname === '/hot') {
                ...
                document.querySelectorAll(className1).forEach(function (item1) {
                    blockKeywords_1(item1, 'h2.ContentItem-title meta[itemprop="name"], meta[itemprop="headline"]');
                });
            }
        }
        ...
        const callback = (mutationsList, observer) => {
            for (const mutation of mutationsList) {
                for (const target of mutation.addedNodes) {
                    if (target.nodeType != 1) return;
                    if (target.className === className2) {
                        blockKeywords_1(target, 'h2.ContentItem-title meta[itemprop="name"], meta[itemprop="headline"]');
                    }
                }
            }
        };
    ...
    function blockKeywords_search() {
        function blockKeywords_now() {
            if (location.search.indexOf('type=content') === -1) return; // 目前只支持搜索页的 [综合]
            document.querySelectorAll('.HotLanding-contentItem, .Card.SearchResult-Card[data-za-detail-view-path-module="AnswerItem"], .Card.SearchResult-Card[data-za-detail-view-path-module="PostItem"]').forEach(function (item1) {
        ...
        const callback = (mutationsList, observer) => {
            ...
            for (const mutation of mutationsList) {
                for (const target of mutation.addedNodes) {
                    if (target.nodeType != 1) return;
                    if (target.className === 'Card SearchResult-Card' && (target.dataset.zaDetailViewPathModule === 'AnswerItem' || target.dataset.zaDetailViewPathModule === 'PostItem')) {
                        blockKeywords_1(target, 'a[data-za-detail-view-id]');
                    }
                }
            }
        };
...
function blockType(type) {
    ...
    if (type === 'search') {
        ...
        if (menu_value('menu_blockTypeSearch') && location.pathname === '/search')
            setTimeout(function () {
                document.querySelector('.RelevantQuery').parentElement.parentElement.hidden = true;
            }, 1000);
    ...
    window.addEventListener('urlchange', function () {
        addSetInterval_(name);
        // 移除相关搜索
        if (menu_value('menu_blockTypeSearch') && location.pathname === '/search' && location.search.indexOf('type=content') > -1)
            setTimeout(function () {
                document.querySelector('.RelevantQuery').parentElement.parentElement.hidden = true;
            }, 1500);
    });
...
function addToQuestion() {
    ...
    if (location.pathname === '/search') {
        ...
        document.querySelectorAll('h2.ContentItem-title a:not(.zhihu_e_tips)').forEach(function (item) {
            addTypeTips_(item);
        });
    }
...
function removeLogin() {
    ...
    if (location.hostname === 'zhuanlan.zhihu.com') {
        // 如果是文章页
        if (!document.querySelector('button.ColumnPageHeader-MenuToggler')) {
            // 未登录
        ...
        // 不是文章页
        if (document.querySelector('button.AppHeader-login')) {
            // 未登录
            const observer = new MutationObserver(removeLoginModal);
            observer.observe(document, { childList: true, subtree: true });
...
function directLink() {
    document.querySelectorAll('a.external[href*="link.zhihu.com/?target="], a.LinkCard[href*="link.zhihu.com/?target="]:not(.MCNLinkCard):not(.ZVideoLinkCard):not(.ADLinkCardContainer)').forEach(function (_this) {
        _this.href = decodeURIComponent(_this.href.substring(_this.href.indexOf('link.zhihu.com/?target=') + 23));
    });
}

// 默认高清原图，修改自：https://greasyfork.org/scripts/402808（从 JQuery 改为原生 JavaScript，且精简、优化了代码）
function originalPic() {
    document.querySelectorAll('img[data-original]:not(.comment_sticker):not(.Avatar)').forEach(function (one) {
...
(function () {
    ...
    window.addEventListener('urlchange', function () {
        // 针对的是从单个回答页跳转到完整回答页时
        if (location.pathname.indexOf('question') > -1 && location.pathname.indexOf('waiting') === -1 && location.pathname.indexOf('/answer/') === -1) {
            //       回答页 //
            setTimeout(function () {
    ...
    function start() {
        ...
        if (location.pathname.indexOf('question') > -1 && location.href.indexOf('/log') == -1) {
            ...
            setInterval(function () {
                topTime_('.ContentItem.AnswerItem, .ContentItem.ArticleItem', 'SearchItem-meta');
            }, 300); // 置顶显示时间
            addTypeTips(); //                                                  区分问题文章
            ...
            if (location.pathname.indexOf('/hot') > -1 || location.href.indexOf('/top-answers') > -1) {
                ...
                setInterval(function () {
                    topTime_('.ContentItem.AnswerItem, .ContentItem.ArticleItem', 'ContentItem-meta');
                }, 300); // 置顶显示时间
                addTypeTips(); //                                              区分问题文章
                addToQuestion(); //                                            直达问题按钮
